#### 7/20/26; 12:15 AM ET by CC

Hardened against posts that carry no markdown, found by running the app against a fork whose posts are written in AsciiDoc (the server renders them to HTML; there is no `markdowntext`). Stock servers can produce such posts too -- any API caller sending only `description`. Three fixes, all verified end to end against a live instance: the console narration in `handleItem` no longer crashes on a markdown-less item (it was throwing before the mirror code ran, so the post never reached WordPress); post content falls back to the item's `description` HTML when there's no markdown, on create and on update -- which had a happy side effect: syntax-highlighted posts arrive on WordPress in full color, since the highlighting rides inline in the HTML; and `findSite` matches feed URLs case-insensitively, the way the server itself treats feed paths, so a URL that works in the browser can't silently fail to match here.
